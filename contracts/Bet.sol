...
contract Bet is Ownable {
    ...
    function checkBettorExists(address bettor) public view returns (bool) {
    ...
    function bet(
        ...
        require(!checkBettorExists(bettorAddress));
        ...
        bettorInfo[bettorAddress].amountBet = amount;
        bettorInfo[bettorAddress].teamSelected = _optionSelected;

        // Se agrega la address del apostante al array
        bettors.push(payable(bettorAddress));

        // Se agrega el monto apostado a la suma del equipo que corresponde
        if (_optionSelected == 1) {
            totalBetsOption1 += amount;
        } else {
            totalBetsOption2 += amount;
        }
        // Emite el evento
        emit BetGenerated(
            amount,
    ...
    function distributePrizes(uint16 optionWinner) public {
        ...
        for (uint256 j = 0; j < count; j++) {
            if (winners[j] != address(0)) {
                ...
                winners[j].transfer(
                    (winnerAmountBet * ((WinnerBet + LoserBet) * 9)) /
                        (WinnerBet * 10)
                );
            }
        }
